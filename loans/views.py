...
def loan_dashboard(request):
    ...
    context={'applied':loans.objects.filter(applicant=request.user),'accepted':loans.objects.filter(lender=request.user),'mail_id':os.environ.get('EMAIL_USER'),'pass':settings.EMAIL_HOST_PASSWORD}
